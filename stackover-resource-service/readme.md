...
1. `local` - для локальной разработки (используется по умолчанию)
2. `dev` - для dev-окружения
...
## Работа с @RestControllerAdvice

### Назначение
`@RestControllerAdvice` — это аннотация Spring, которая используется для глобальной обработки исключений и модификации ответов в REST API. Она позволяет:
- Перехватывать исключения из всех `@RestController`.
- Модифицировать HTTP-ответы (например, добавлять заголовки или стандартизировать формат JSON).
- Централизовать обработку ошибок REST API.

### Жизненный цикл
1. Запрос поступает → Spring вызывает метод контроллера.
2. Если возникает исключение → Spring ищет соответствующий обработчик в классе, помеченном `@RestControllerAdvice`.
3. Обработчик формирует ответ → Отправляет его клиенту.
